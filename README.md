...
| <a name="input_gateway_api_config_channel"></a> [gateway\_api\_config\_channel](#input\_gateway\_api\_config\_channel) | The Gateway API channel to use for the cluster. Valid values are CHANNEL\_DISABLED, CHANNEL\_EXPERIMENTAL, CHANNEL\_STANDARD. Set to null to disable the Gateway API configuration block. | `string` | `null` | no |
